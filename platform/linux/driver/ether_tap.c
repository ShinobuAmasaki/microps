...
ether_tap_addr(struct net_device *dev)
{
   ...
   // ioctl()のSIOCGFHWADDR要求がソケットとして開かれたディスクリプタでのみ有効なため、ソケットを開いている

   memcpy(dev->addr, ifr.ifr_hwaddr.sa_data, ETHER_ADDR_LEN); // 取得したアドレスをデバイス構造体へコピーする
   close(soc);
   return 0;
}
...
ether_tap_open(struct net_device *dev)
{
   ...
   strncpy(ifr.ifr_name, tap->name, sizeof(ifr.ifr_name)-1); // TAPデバイスの名前を設定する。
   ifr.ifr_flags = IFF_TAP | IFF_NO_PI;  //フラグ（TAPモードで、パケット情報ヘッダをつけない）
...
ether_tap_isr(unsigned int irq, void *id)
{
   ...
   pfd.events = POLLIN;
   while(1) {
      ret = poll(&pfd, 1, 0); // タイムアウト時間を0に設定したpoll()で読み込み可能なデータの存在を確認する
      if (ret == -1) {
         if (errno == EINTR) {
            continue; // errnoがEINTRの場合は再試行する（シグナルに割り込まれたという回復可能なエラー）
         }
         errorf("poll: %s, dev=%s", strerror(errno), dev->name);
         return -1;
      }
      if (ret == 0) { // 戻り値が0だったらタイムアウト（読み込み可能データなし）
...
ether_tap_init(const char *name, const char *addr)
{
   ...
   // Ethernetデバイス共通のパラメータを設定する
   ether_setup_helper(dev);

   // 引数でハードウェアアドレスの文字列が渡されたら、それをバイト列に変換してセットする
   if (addr) {
   ...
   dev->ops = &ether_tap_ops; // ドライバの関数群を設定する

   // ドライバ内部で使用するプライベートなデータを生成して保持する
